...
class PPOBuffer:
    ...
    def get_minibatch(self, batch_size, device):
        batch_indices = np.random.choice(
            len(self.states), batch_size, replace=False)
        
        batch_states = torch.stack(
            [self.states[i] for i in batch_indices]).to(device)
        batch_actions = torch.stack(
            [self.actions[i] for i in batch_indices]).to(device)
        batch_log_probs = torch.stack(
            [self.log_probs[i] for i in batch_indices]).to(device)
...
def train(ppo, env, device):
    ...
    optimizer = torch.optim.Adam([
        {'params': ppo._policy.parameters(), 'lr': 3e-4},
        {'params': ppo._value.parameters(), 'lr': 3e-3},
    ])
